...
import { resetCalendarEvents } from '@/src/reducers/calendar/calendarSlice';
import { setDB } from '@/src/reducers/dbSlice';
import { setCurrentUser } from '@/src/reducers/userSlice';
import {
  getInitialEventGroups,
  getInitialEventInvites,
  getInitialEvents,
  getInitialInvited,
  getInitialUserEvents,
  getInitialUsers,
} from '@/src/utils/data';
      ...
      <div className='flex items-center justify-center w-screen min-h-screen bg-gray-400 '>
        <div className='max-w-screen-sm p-6 bg-white rounded-md shadow-md '>
          ...
          <p className='mt-4 text-gray-600'>
            This project is an offline version where the data is pre-populated
            using Redux in the beginning. So, sending emails will only be
            allowed amongst the users below.
          </p>
          <div className='mt-8'>
                ...
                return (
                  ...
                  >
                    <div className='flex'>
                      ...
                      />
                      <p className='ml-3'>{displayName}</p>
                    </div>
                    <p className='ml-3 text-gray-400'>{email}</p>
                  </div>
                );
              })}
            </div>
