...
const CalendarViewGridItem = ({
  ...
  useEffect(() => {
    if (gridEventContainerRef && gridEventContainerRef.current) {
      const height =
        gridEventContainerRef.current.getBoundingClientRect().height;
  ...
  const gridItemEvents = (events, containerHeight) => {
    ...
    return events.slice(0, maxNumOfShowingEvents).map((e, index) => {
      if (e.isEmpty) {
        return (
          <div
            key={`empty-${index}`}
        ...
        if (e.duration > -1) {
          ...
          return (
            <div key={e.eventUid}>
              <CalendarViewGridItemSingleEvent index={index} data={e} />
            </div>
          );
        }
      }
  ...
  return (
    <div className='flex flex-col bg-white'>
          ...
          className={`
            ...
            ${
              isSameDay(today, set(new Date(), { month, date }))
                ...
                : month === getMonth(new Date(targetDate))
                ? 'text-gray-700 font-medium hover:bg-gray-100'
                : 'hover:bg-gray-100 text-gray-400 font-medium'
            }
          `}
      ...
      <div ref={gridEventContainerRef} className='relative flex-auto'>
        {gridItemEvents(events, gridEventContainerHeight)}
        {Math.floor(gridEventContainerHeight / 23) - 2 < events.length && (
          <div className='px-2 py-1 mt-1 mr-2 text-xs font-semibold transition cursor-pointer rounded-r-md hover:bg-gray-100'>
            <span>
              {events.length - (Math.floor(gridEventContainerHeight / 23) - 2)}{' '}
              more
            </span>
          </div>
        )}
