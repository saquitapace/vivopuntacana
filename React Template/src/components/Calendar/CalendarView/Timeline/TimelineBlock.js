...
const TimelineBlock = ({
  ...
  const getBlockHeight = (event) => {
    if (!isMultiday) {
      ...
      const start = set(new Date(event.startDate), {
        hours: startHour,
        minutes: startMinuteSeg,
      });
      const end = set(new Date(event.endDate), {
        hours: endHour,
        minutes: endMinuteSeg,
      });

      return (
        Math.floor(differenceInMinutes(end, start) / 15) * basePixel +
        (Math.floor(differenceInMinutes(end, start) / 15) - 1)
      );
    } else {
      return 22;
    }
  ...
  const blockSubtitle = (blockHeight, isMultiday) => {
    console.log('eventStartDateError ', event);
